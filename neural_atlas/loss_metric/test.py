...
class Test:
    def __init__(
        ...
        f_score_dist_threshold = metric_config.f_score_scale_ratio \
                                 * pcl_normalization_scale
        overlap_dist_threshold = metric_config.overlap_scale_ratio \
                                 * pcl_normalization_scale
        self.metric = metric.Metric(
            num_charts,
            uv_space_scale,
            eval_target_pcl_nml_size,
            metric_config.default_chamfer_dist,
            metric_config.default_normal,
            metric_config.default_f_score,
            metric_config.default_distortion,
            metric_config.default_stitching_err,
            f_score_dist_threshold,
            metric_config.distortion_eps,
            metric_config.degen_chart_area_ratio,
    ...
    def test_step(self, batch):
        ...
        with torch.enable_grad():
            # enable grad to calc. distortion & normal consistency metrics
            batch.mapped_pcl, batch.graph_wght = self.infer_pcl(                # (num_charts, batch.size, batch.chart_uv_sample_size, 3), (batch.size, batch.chart_uv_sample_size_sqrt, batch.chart_uv_sample_size_sqrt, 4)
        ...
        for sample.index in range(batch.size):
            ...
            with torch.cuda.stream(sample.stream):
                ...
                sample.graph_wght = batch.graph_wght[sample.index, ...]         # (batch.chart_uv_sample_size_sqrt, batch.chart_uv_sample_size_sqrt, 4)
    ...
    def infer_latent_code(self, batch):
        ...
        latent_code.common = self.model.encoder(dataset_input_data)             # (batch.size, latent_dims)
    ...
    def infer_pcl(
        self,
        latent_code_cond_homeomorphism,                                         # (1, batch.size, latent_dims)
        input_pcl                                                               # (1, batch.size, P, 3)
    ):
        latent_code_cond_homeomorphism = latent_code_cond_homeomorphism[0, ...] # (batch.size, latent_dims)
        input_pcl = input_pcl[0, :, :, :2]                                      # (batch.size, P, 2)
        
        rec_pre2 = self.model.decoder.folding(                                  # (batch.size, P, 3)
            latent_code_cond_homeomorphism, input_pcl
        )
        grid = self.model.decoder.tearing(                                      # (batch.size, P, 2)
            latent_code_cond_homeomorphism, input_pcl, rec_pre2
        )
        rec_pre1 = self.model.decoder.folding(                                  # (batch.size, P, 3)
            latent_code_cond_homeomorphism, grid
        )
        rec, graph_wght = self.model.decoder.graph_filter(grid, rec_pre1)       # (batch.size, P, 3), (batch.size, 4, sqrt(P), sqrt(P))

        deformed_pcl = rec.unsqueeze(dim=0).clone()                             # (1, batch.size, P, 3)
        graph_wght = graph_wght.permute([0, 2, 3, 1])                           # (batch.size, sqrt(P), sqrt(P), 4)
        return deformed_pcl, graph_wght
